...
#------------------[ LOGO-LAKNAT ]-----------------#
def banner():
	print(f'{b}\t      SIMPLE CRACK')
#  print(f'\t THE MEMEKERS ')
	print(f'\t  Author ├──> RANDY ')
	print(f'\t    From ├──> SUMATRA ')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
		...
		try:
			...
			print(f'[!] JARINGAN EROR BRO COBA LAGI !{x}')
			exit()
	except IOError:
		login_bas()
def login_bas():
...
def menu(bas_id):
	try:
		...
		print(f'[×] Cookies Telah Kadaluarsa ! Ganti Cookies !{x} ')
		time.sleep(5)
		login_bas()
	os.system('clear')
	banner()
	ip = requests.get("https://api.ipify.org").text
	alvino_xy(f'{P}├──>  ID  : '+str(bas_id))
	alvino_xy(f'{P}├──>  IP  : {ip}{x}')
# print(f'>> Github   : {gh}')
	print('')
...
# print('├──> 4. Crack File	')
	...
	if _____alvino__adijaya_____ in ['1']:
		...
		back()
def error():
	print(f'{b}├──> Maaf Fitur Ini Masih Di Perbaiki {b}')
	time.sleep(4)
	back()
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	print(f'├──> 1. Hasil {h}OK{b} Anda ')
	print(f'├──> 2. Hasil {k}CP{b} Anda ')
	print('├──> 3. Kembali	')
	kz = input(f'\n├──> Pilih : ')
	if kz in ['2']:
		...
		if len(vin)==0:
			...
			for isi in vin:
				...
				if cih<10:
					...
					print(f'├──> %s. %s ({k} %s {b}Idz )'%(nom,isi,len(hem)))
				else:
					lol.update({str(cih):str(isi)})
			...
			for cpku in range(len(lin)):
				...
				print(f'{b}├──> {k}{cpkuni[0]}|{cpkuni[1]}')
				nocp +=1
			print('')
			input(f'{b}[{m} Klik Enter{x} ]')
			back()
	elif kz in ['1']:
...
def dump_massal():
	...
	try:
		...
		print(f'├──>{k} Pertemanan Tidak Public {b}')
		time.sleep(3)
		back()
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'├──> {b}•{b}•{b}•{b} Sedang Menggenjot Istri Orang {b}•{b}•{b}•{b} <<<<< ')
	print('')
	print(f'├──> Hasil {h}OK{b} Tersimpan Di : {h}OK/%s {b}'%(okc))
	print(f'├──> Hasil {k}CP{b} Tersimpan Di : {k}CP/%s {b}'%(cpc))
		...
		#		os.popen('')
				...
				#	os.popen('')
					...
					for muncul in apkaktif:
						infoakun+= (f"	{b}[{h}{nok}{x}] {b}{muncul[0]} {muncul[1]}{x}\n")
						nok+=1
					...
					for muncul in apkexp:
						hit+=1
						infoakun += (f"	{b}[{k}{hit}{b}] {m}{muncul[0]} {muncul[1]}{b}\n")
